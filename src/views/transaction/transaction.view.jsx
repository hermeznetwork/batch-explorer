...
function Transaction({ onLoadTransaction, transactionTask }) {
  ...
  function getTransactionTypeLabel(transactionType) {
    switch (transactionType) {
      ...
      case TxType.ForceExit: {
        return (
          <>
            Exit (Initiate withdraw){" "}
            <p className={classes.exitAdditionalMessage}>(Withdrawal step 1 of 2 was completed)</p>
          </>
        );
      }
  ...
  function showBJJ(type) {
    if (type === "from") {
      return (
        (transactionTask.data.fromBjj && transactionTask.data.fromBjj !== EMPTY_BJJ_ADDR) ||
        (transactionTask.data.fromBJJ && transactionTask.data.fromBJJ !== EMPTY_BJJ_ADDR)
      );
    }
    if (type === "to") {
      return (
        (transactionTask.data.toBjj && transactionTask.data.toBjj !== EMPTY_BJJ_ADDR) ||
        (transactionTask.data.toBJJ && transactionTask.data.toBJJ !== EMPTY_BJJ_ADDR)
      );
    }
  }
  ...
  function showHezEthereumAddress(type) {
    if (type === "from") {
      return (
        ...
        transactionTask.data.fromHezEthereumAddress !== INTERNAL_ACCOUNT_ETH_ADDR &&
        !showBJJ("from")
      );
    }
    if (type === "to") {
      return (
        ...
        transactionTask.data.toHezEthereumAddress !== INTERNAL_ACCOUNT_ETH_ADDR &&
        !showBJJ("to")
      );
    }
  }
              ...
              case "successful": {
                return (
                  <section>
                    <Row>
                      ...
                      <Col>
                        <Row wrapped>
                          ...
                          <Col wrapped>{transactionTask.data.id}</Col>
                        </Row>
                      </Col>
                    </Row>
                    <Row>
                    ...
                    <Row>
                      <Col>Timestamp</Col>
                      <Col>{getTimeZoneTimestamp(transactionTask.data.timestamp)}</Col>
                    </Row>
                    <Row>
                      <Col>Type of transaction</Col>
                      <Col>{getTransactionTypeLabel(transactionTask.data.type)}</Col>
                    </Row>
                        ...
                        <Col link>
                          <Row wrapped>
                            <CopyToClipboardButton
                              content={transactionTask.data.fromHezEthereumAddress}
                            />
                            <Col wrapped>
                              <Link
                                to={`/user-account/${transactionTask.data.fromHezEthereumAddress}`}
                              >
                                {transactionTask.data.fromHezEthereumAddress}
                              </Link>
                            </Col>
                    ...
                    {showBJJ("from") ? (
                      <Row>
                        ...
                        <Col link>
                          <Row wrapped>
                            <CopyToClipboardButton
                              content={transactionTask.data.fromBjj || transactionTask.data.fromBJJ}
                            />
                            <Col wrapped>
                              <Link
                                to={`/user-account/${
                              ...
                              <p className={classes.bjjAddressDescription}>
                                (Hermez internal address)
                              </p>
                            </Col>
                          </Row>
                        </Col>
                    ...
                    {showHezEthereumAddress("to") ? (
                      <Row>
                        ...
                        <Col link>
                          <Row wrapped>
                            <CopyToClipboardButton
                              content={transactionTask.data.toHezEthereumAddress}
                            />
                            <Col wrapped>
                              <Link
                                to={`/user-account/${transactionTask.data.toHezEthereumAddress}`}
                              >
                                {transactionTask.data.toHezEthereumAddress}
                              </Link>
                            </Col>
                    ...
                    {showBJJ("to") ? (
                      <Row>
                        ...
                        <Col link>
                          <Row wrapped>
                            <CopyToClipboardButton
                              content={transactionTask.data.toBjj || transactionTask.data.toBJJ}
                            />
                            <Col wrapped>
                              <Link
                                to={`/user-account/${
                    ...
                    <Row>
                      ...
                      <Col>
                        ...
                        {transactionTask.data.token.symbol}
                      </Col>
                    </Row>
                    {transactionTask.data.type === TxType.CreateAccountDepositTransfer ||
                    transactionTask.data.type === TxType.DepositTransfer ? (
                      <Row>
                        <Col>Deposit Amount</Col>
                        <Col>
                          {getFixedTokenAmount(
                            getTransactionDepositAmount(transactionTask.data),
                            transactionTask.data.token.decimals
                          )}{" "}
                          {transactionTask.data.token.symbol}
                        </Col>
                    ...
                    {transactionTask.data.fee || transactionTask.data.L2Info?.fee ? (
                      <Row>
                        ...
                        <Col>
                          ${" "}
                          {transactionTask.data.L2Info?.historicFeeUSD
                            ? Number(transactionTask.data.L2Info.historicFeeUSD).toFixed(2)
                            : getFeeInUsd(
                                transactionTask.data.fee || transactionTask.data.L2Info?.fee,
                                transactionTask.data.amount,
                                transactionTask.data.token
                              )}
                        </Col>
                      </Row>
                    ) : (
                    ...
                    >
                      {Number.isInteger(transactionTask.data.slot) ? (
                        <Row>
                          <Col>Slot</Col>
                          <Col link>
                            <Link to={`/slot/${transactionTask.data.slot}`}>
                      ...
                      {Number.isInteger(transactionTask.data.position) ? (
                        <Row>
                          <Col>Position in batch</Col>
                          <Col>{transactionTask.data.position}</Col>
                        </Row>
                      ) : (
                        <></>
                      )}
